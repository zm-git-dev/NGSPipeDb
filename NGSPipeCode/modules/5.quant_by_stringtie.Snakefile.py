...
rule reStringtie_by_stringtie:
    ...
    shell:
        '''
        stringtie -p {threads} -G {input.mergedGtf} -o {output.gtf} -A {output.expr} -B -e -l {wildcards.sample} {input.sorted_bam};
...
rule countMatrix_by_stringtie:
    ...
    shell:
        ...
        # TPM
        python NGSPipeCode/script/stringtie_expression_matrix.py -m tpm -s {quantify_outdir} -t {quantify_outdir}/transcript_tpm_all_samples.tsv -g {quantify_outdir}/gene_tpm_all_samples.tsv >>{log} 2>&1;
        # FPKM
        python NGSPipeCode/script/stringtie_expression_matrix.py -m fpkm -s {quantify_outdir} -t {quantify_outdir}/transcript_fpkm_all_samples.tsv -g {quantify_outdir}/gene_fpkm_all_samples.tsv >>{log} 2>&1;
        # Normalizition
        '''
